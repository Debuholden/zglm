...
pub const Vec3 = struct {
    ...
    // Returns the max value between [self] and [other]
    pub fn max(self: Vec3, other: Vec3) Vec3 {
        var cglm_vec3: c.vec3 align(16) = .{ self.x, self.y, self.z };

        c.glm_vec3_maxv(self._cglm_vec3, other._cglm_vec3, cglm_vec3);

        return cglm_vec3;
    }

    // Returns the min value between [self] and [other]
    pub fn min(self: Vec3, other: Vec3) Vec3 {
        var cglm_vec3: c.vec3 align(16) = .{ self.x, self.y, self.z };

        c.glm_vec3_minv(self._cglm_vec3, other._cglm_vec3, cglm_vec3);
